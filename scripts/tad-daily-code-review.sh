...
2. First, DROP silently — before any severity routing — any finding that merely re-raises a PRE-EXISTING, already-parked structural or design item: one matching a \`docs/decisions.md\` § Accepted / Won't-Fix entry (revisit trigger unfired), or a TAD task-pipeline item that predates this diff (a \`tasks/*.md\` file incl. \`status: later\`, or a \`tasks/_backlog.md\` one-liner). Capture is acknowledgement — re-raising a parked item is the review fatigue this sweep exists to avoid. CARVE-OUT — never drop a finding that identifies a concrete NEW correctness defect introduced by the reviewed diff, even if it lives in a file some task or backlog line mentions; a new bug is never "already captured", and surfacing it is exactly this sweep's job. Then route every REMAINING finding by these rules and NOTHING else:
