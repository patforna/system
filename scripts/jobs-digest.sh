...
# scratch state. Seen 2026-08-20, before scratch paths were per-run: the losing
# run died on a missing merged.json, then validated its scoring against a
# swapped candidate set, burned three attempts, sent nothing, and escalated.
#
...
NOOP_FLAG="/tmp/dagu-noop-${DAG_NAME:-jobs-digest}"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  ...
  if (( held )); then
    touch "$NOOP_FLAG"
    echo "another jobs-digest run (pid ${holder:-starting}) is in flight — skipping" >&2
    exit 0
  fi
  echo "clearing stale jobs-digest lock (pid ${holder:-unknown}, age ${age}s)" >&2
  rm -rf "$LOCK_DIR"
  if ! mkdir "$LOCK_DIR" 2>/dev/null; then
    touch "$NOOP_FLAG"
    echo "another jobs-digest run took the lock first — skipping" >&2
    exit 0
  fi
fi
echo $$ > "$LOCK_DIR/pid"
